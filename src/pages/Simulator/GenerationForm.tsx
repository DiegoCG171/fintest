...
const columns = ["", "Campo", "Nombre", "Función", "Parámetro", ""];
...
const SubFieldTable: React.FC<SubFieldTableProps> = ({
  ...
  const renderSelectOptions = (options: string[]) =>
    options.map((option) => (
      <MenuItem key={option} value={option.toLocaleLowerCase()} sx={{ fontSize: "0.6rem" }}>
        {option}
      </MenuItem>
    ));
...
export const GenerationForm = ({setParentRows}: Props) => {
  ...
  useEffect(() => {
    const newRows = rules[0].fields.map((field) => {
      const dataRow = templates[0].generationTransaction.find(
  ...
  return (
    <TableContainer
      component={Paper}
      sx={{
        maxHeight: { xs: "calc(50vh + 2px)", xl: "calc(60vh + 2px)", lg: "calc(50vh + 2px)" },
        overflowY: "auto",
      }}
    >
